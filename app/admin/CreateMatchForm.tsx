...
export function CreateMatchForm({
  ...
  const [matchType, setMatchType] = useState<MatchType>("stableford_2v2");
  ...
  const oneV1 = isMatchPlay1v1(matchType);
  ...
  return (
    <form action={formAction} className="space-y-3 max-w-lg">
      ...
      <div className="flex gap-4 flex-wrap">
        ...
        <label className="flex items-center gap-1 text-sm">
          <input
            type="radio"
            name="match_type_radio"
            checked={matchType === "saturday_match_play_1v1"}
            onChange={() => setMatchType("saturday_match_play_1v1")}
          />
      ...
      {matchType === "saturday_match_play_1v1" ? (
        <div className="grid grid-cols-2 gap-4 text-sm">
          <div>
            <p className="font-medium text-slate-700 mb-1">Player 1</p>
            <select name="team_a_player_1" className="w-full px-2 py-1 border rounded" required>
          ...
          <div>
            <p className="font-medium text-slate-700 mb-1">Player 2</p>
            <select name="team_b_player_1" className="w-full px-2 py-1 border rounded" required>
              <option value="">Select player</option>
              {playersSorted.map((p) => (
                <option key={p.id} value={p.id}>{p.name}</option>
              ))}
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          <div>
            ...
            <select name="team_a_player_1" className="w-full px-2 py-1 border rounded mb-1" required>
          ...
          <div>
            ...
            <select name="team_b_player_1" className="w-full px-2 py-1 border rounded mb-1" required>
              <option value="">Player 1</option>
              {mcavoyPlayers.map((p) => (
                <option key={p.id} value={p.id}>{p.name}</option>
              ))}
